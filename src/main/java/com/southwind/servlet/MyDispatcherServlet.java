...
public class MyDispatcherServlet extends HttpServlet{
	...
	public void init(ServletConfig config) throws ServletException {
		...
		out.printf("init %s %s called %n", "MyDispatcherServlet",  "MyDispatcherServlet2");
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		out.printf("doPost %s %s called %n", "MyDispatcherServlet",  "MyDispatcherServlet2");

		//获取请求
        String[] urlParts = req.getRequestURI().split("/");
		String handlerUri = urlParts[urlParts.length - 2];

		//获取Controller实例
		Object obj = iocContainer.get(handlerUri);

		String methodUri = urlParts[urlParts.length - 1];
		//获取业务方法
		...
		try {
			...
			//页面跳转
            String page1 = "/kinglion.html";
            String page2 = "/a2/anything";
            String page3 = "/windgrass.jsp1";
//            result = "/index.jsp";
			RequestDispatcher rd = req.getRequestDispatcher(page2);

            //resp.getWriter().format("%s%n", "Blue Alphabetic Moon");
			//rd.forward(req, resp);
			//org.eclipse.jetty.io.RuntimeIOException: org.eclipse.jetty.io.EofException: Stream closed
            //resp.getWriter().format("%-20.10s", "Icnnotsee!");

			rd.include(req, resp);

			rd = req.getRequestDispatcher(page1);
			rd.include(req, resp);

            rd = req.getRequestDispatcher(page3);
            rd.include(req, resp);

			//resp.getWriter().format("%-10.8s", "Isee!");

			//print two "Hello World! From Test Page!!"s
//			rd.include(req, resp);
//			rd.include(req, resp);

//			req.getRequestDispatcher(result).include(req, resp);
			out.format("%nThen see this!!");
		} catch (Exception e) {
